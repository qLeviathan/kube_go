...
type Proof struct {
	...
	Complete   bool
	MaxDepth   int
	Unfolding  int // expansion between levels, must be ≤ 10
}
...
// Iterative only — no recursion.
func (pb *ProofBuilder) BuildProof(cr kinds.ComposedResult) Proof {
	pb.proofCounter++
	proof := Proof{
		ID:    fmt.Sprintf("proof-%d", pb.proofCounter),
		Steps: make([]ProofStep, 0, 20),
	}
	...
	// Level 2: ML proof trace (unfolded)
	for i := 0; i < len(cr.MLResult.ProofTrace) && i < 10; i++ {
		stepNum++
		proof.Steps = append(proof.Steps, ProofStep{
			Level:      2,
			StepNumber: stepNum,
	...
	// Level 2: AR proof trace (unfolded)
	for i := 0; i < len(cr.ARResult.ProofTrace) && i < 10; i++ {
		stepNum++
		proof.Steps = append(proof.Steps, ProofStep{
			Level:      2,
			StepNumber: stepNum,
	...
	if cr.MLResult.Prediction == cr.ARResult.Prediction {
		...
		proof.Steps = append(proof.Steps, ProofStep{
			...
			StepNumber: stepNum,
			Type:       "rule",
			Content:    fmt.Sprintf("COMPOSE: ML=%q AR=%q disagree => AR priority (higher assurance), final=%q", cr.MLResult.Prediction, cr.ARResult.Prediction, cr.Final),
			Justified:  true,
		})
	}
...
func classifyTraceStep(s string) string {
	if strings.HasPrefix(s, "premise:") || strings.HasPrefix(s, "fact:") {
		return "premise"
	}
	if strings.HasPrefix(s, "rule[") || strings.HasPrefix(s, "derived:") {
		return "rule"
	}
	if strings.HasPrefix(s, "conclusion:") {
		return "conclusion"
	}
	if strings.HasPrefix(s, "evidence:") || strings.HasPrefix(s, "prior(") {
		return "evidence"
	}
	if strings.HasPrefix(s, "propagate(") {
		return "propagation"
	}
	return "premise"
}
...
func computeMaxUnfolding(steps []ProofStep) int {
	// Count max consecutive steps at the same level (unfolding expansion)
	if len(steps) == 0 {
		return 0
	}
	maxCount := 0
